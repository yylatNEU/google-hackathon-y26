...
ROLE_AUTH_SECRET_RESOURCE_NAME="${PARKPULSE_ROLE_AUTH_SECRET_SECRET:-parkpulse-role-auth-secret}"
ROLE_ISSUER_KEY_RESOURCE_NAME="${PARKPULSE_ROLE_ISSUER_KEY_SECRET:-parkpulse-role-issuer-key}"
...
if gcloud secrets describe "$ROLE_AUTH_SECRET_RESOURCE_NAME" --project "$PROJECT_ID" >/dev/null 2>&1; then
  gcloud secrets add-iam-policy-binding "$ROLE_AUTH_SECRET_RESOURCE_NAME" \
    --project "$PROJECT_ID" \
    --member "serviceAccount:${SERVICE_ACCOUNT_EMAIL}" \
    --role roles/secretmanager.secretAccessor \
    --quiet >/dev/null || true
  DEPLOY_ARGS+=(--set-secrets "PARKPULSE_ROLE_AUTH_SECRET=${ROLE_AUTH_SECRET_RESOURCE_NAME}:latest")
fi
if gcloud secrets describe "$ROLE_ISSUER_KEY_RESOURCE_NAME" --project "$PROJECT_ID" >/dev/null 2>&1; then
  gcloud secrets add-iam-policy-binding "$ROLE_ISSUER_KEY_RESOURCE_NAME" \
    --project "$PROJECT_ID" \
    --member "serviceAccount:${SERVICE_ACCOUNT_EMAIL}" \
    --role roles/secretmanager.secretAccessor \
    --quiet >/dev/null || true
  DEPLOY_ARGS+=(--set-secrets "PARKPULSE_ROLE_SESSION_ISSUER_KEY=${ROLE_ISSUER_KEY_RESOURCE_NAME}:latest")
fi
