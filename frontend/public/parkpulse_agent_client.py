...
class ParkPulseAgentClient:
    ...
    def post(self, path: str, body: dict[str, Any], expected_status: int = 200) -> dict[str, Any]:
        request = Request(
            f"{self.api_url}{path}",
    ...
    def get(self, path: str, query: dict[str, Any] | None = None, expected_status: int = 200) -> dict[str, Any]:
        suffix = f"?{urlencode(query)}" if query else ""
        request = Request(f"{self.api_url}{path}{suffix}", headers={"accept": "application/json"}, method="GET")
        try:
            with urlopen(request, timeout=self.timeout_seconds) as response:
                payload = json.loads(response.read().decode("utf-8") or "{}")
                status_code = response.status
    ...
    def revoke_certification_credential(self, credential: dict[str, Any], reason: str = "revoked_by_external_demo") -> dict[str, Any]:
        return self.post("/api/park/agent-onboarding/revoke-credential", {"credential": credential, "reason": reason})
